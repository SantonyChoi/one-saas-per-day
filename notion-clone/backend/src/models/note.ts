...
export const createNote = (noteData: NoteInput): number => {
  ...
  // Convert is_public to 0 or 1 for SQLite boolean
  const isPublicValue = is_public === true ? 1 : 0;
...
export const updateNote = (id: number, noteData: NoteUpdate): boolean => {
  ...
  // Convert is_public to 0 or 1 for SQLite boolean if it's defined
  let isPublicValue = undefined;
  if (is_public !== undefined) {
    isPublicValue = is_public === true ? 1 : 0;
  }
